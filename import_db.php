...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // 1. ACCIÓN: SUBIR ARCHIVO, CONTAR TABLAS Y LIMPIAR BASE DE DATOS
    ...
    // 2. ACCIÓN: PROCESAMIENTO POR LOTES Y CONTEO DE TABLAS
    if ($_POST['action'] === 'process_batch') {
        $offset = intval($_POST['offset']);
        $current_table = isset($_POST['currentTable']) ? $_POST['currentTable'] : 'Iniciando...';$created_tables_count = isset($_POST['createdTablesCount']) ? intval($_POST['createdTablesCount']) : 0;
        $dump_path =$target_dir . 'database_backup.sql';

        if (!file_exists($dump_path)) {
            echo json_encode(array('success' => false, 'error' => 'No se encuentra el archivo SQL subido.'));
            exit;
        }

        $mysqli = new mysqli($db_host, $db_user,$db_pass, $db_name, (int)$db_port);
        if ($mysqli->connect_error) {
            echo json_encode(array('success' => false, 'error' => 'Error de conexión a MariaDB: ' . $mysqli->connect_error));
            exit;
        }
        $mysqli->set_charset("utf8mb4");
        
        // Ajustar límites de sesión de MariaDB
